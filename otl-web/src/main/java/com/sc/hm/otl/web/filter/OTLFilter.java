 ...
 * <br/>
 * The <code>OTLFilter</code> will intercept any incoming http request and  set
 ...
 * </pre>
 *
 * This will enable container to invoke the {@link OTLFilter} only when the requested
 ...
 *         .addFilter("OpenTracingFilter", "com.sc.hm.otl.web.filter.OTLFilter");
 *
 ...
 * </pre>
 *
 * The above code will register the OpenTracingFilter with the container.
 * Note that it is not mandatory to provide a custom decorator or skip pattern.
 *
 * <p>
 * <br/>
 * <b>3.</b> For spring boot, use the below code to setup the {@code OTLFilter}.
 *
 * <pre>
 * {@code
 * @Bean
 * public FilterRegistrationBean registerOpenTracingFilter() {
 *     Tracer tracer = GlobalTracer.get();
 *     FilterSpanDecorator spanDecorator = new OTLFilterSpanDecorator(tracer);
 *
 *     FilterRegistrationBean registration = new FilterRegistrationBean(
 *         new OTLFilter(tracer, spanDecorator, null, null));
 ...
 */
@WebFilter
public class OTLFilter implements Filter {
    
    private final Logger logger = LoggerFactory.getLogger(OTLFilter.class);
    
    private final Tracer tracer = GlobalTracer.get();
    private final List<FilterSpanDecorator> decorators = new ArrayList<>();
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        decorators.add(new StandardFilterSpanDecorator());
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Initialized the OTLFilter with"
    ...
    public void doFilter(ServletRequest request
        ...
        SpanContext context = tracer.extract(Format.Builtin.TEXT_MAP_EXTRACT
            , new HttpRequestAdapter(httpRequest));
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("OTL Servlet filter created new Span: {}", span.context());
        }
        
        for (FilterSpanDecorator decorator : decorators) {
    ...
    public void destroy() {
        if (logger.isTraceEnabled()) {
            logger.trace("OTLFilter is destroyed");
        }
    }
     ...
     */
    private FilterSpanDecorator initDecorator(String decoratorClass) {
