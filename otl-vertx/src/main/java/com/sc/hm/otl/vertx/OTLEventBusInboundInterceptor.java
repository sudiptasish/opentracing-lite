 ...
 */
public class OTLEventBusInboundInterceptor<T> implements Handler<DeliveryContext<T>> {
    
    private final Logger logger = LoggerFactory.getLogger(OTLEventBusInboundInterceptor.class);
    
    private final Tracer tracer = GlobalTracer.get();
    
    private final List<VertxEventBusSpanDecorator> decorators = new ArrayList<>();
    
    OTLEventBusInboundInterceptor() {
    ...
    public void handle(DeliveryContext<T> deliveryCtx) {
        ...
        SpanContext context = tracer.extract(Format.Builtin.TEXT_MAP_EXTRACT
                , new VertxEventBusCtxCarrier(deliveryCtx.message().headers()));
